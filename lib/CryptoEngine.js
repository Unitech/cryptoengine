...
var bitcoin_rpc = require('bitcoin');
    each = require('each'),
    ev2 = require('events').EventEmitter,
    _ = require('underscore'),

    db = new require('../api/database'),

    VerifyTransactions = require('./VerifyTransactions'),
    MonitorTransactions = require('./MonitorTransactions'),
    GlobalStats = require('./GlobalStats');
...
var CryptoEngine = (function () {
    ...
    function cls(opts) {
        var crypto_daemon = new bitcoin_rpc.Client(opts.crypto_daemon),
            channel = new ev2(),
            verifytransactions = new VerifyTransactions(crypto_daemon, opts, channel, db),
            monitortransactions = new MonitorTransactions(crypto_daemon, opts, channel, db),
            globalstats = new GlobalStats(crypto_daemon, opts, channel, db),

            debug = require('debug')('crypto:engine:' + opts.instance_name);
        ...
        function init(cb) {
            db.construct(opts.database,function(err){
                if(err) throw new Error(err);
